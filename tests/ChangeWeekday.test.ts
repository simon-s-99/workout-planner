...
test.describe("Change weekday tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("http://localhost:5173");
  });

  test(
    "add pushups as an exercise, then switch weekday from monday to friday and add hip thrusts as an exercise",
    async ({ page }) => {
      ...
      await page.click('.WeekdayLabel:text("Friday")'); //friday
